...
# Install lumen dependencies if lumen is already installed
if [ -f /vagrant/public/index.php ]; then
  su - vagrant -c "cd /vagrant && composer install"

# Otherwise
else
  # Download latest version of lumen
  su - vagrant -c "cd /tmp && wget http://cabinet.laravel.com/latest_lumen.zip && unzip latest_lumen.zip -d lumen && rm latest_lumen.zip"
  rm /tmp/lumen/*.md
